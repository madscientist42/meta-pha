...
ruby_do_compile() {
	for gem in ${RUBY_BUILD_GEMS}; do
		${RUBY_COMPILE_FLAGS} ${RECIPE_SYSROOT_NATIVE}/usr/bin/gem build $gem
	done
}
...
ruby_do_install() {
	for gem in ${RUBY_INSTALL_GEMS}; do
		${RECIPE_SYSROOT_NATIVE}/usr/bin/gem install --ignore-dependencies --local --env-shebang --install-dir ${D}/${libdir}/ruby/gems/${RUBY_GEM_VERSION}/ $gem
	done
